...
module Kit
  module Amazon
    ...
    def self.create_instance(site, type, host, image)
      instance_type = host['instance_type']
      user = host['user'] || 'ubuntu'
      key = KEY_PAIRS[site] || site

      cmd = "bundle exec knife ec2 server create -f #{instance_type} -I #{image} -Z #{host['zone']} -S #{key} -G #{host['security_groups']} -N #{site}-#{type} --ssh-user=#{user} -i #{host['ssh_key']}"
